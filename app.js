...
async function loadMainMenu() {
  await inquirer.prompt(questions.mainMenu).then(async function (answers) {
...
async function viewAllRoles() {
  await myBLL.viewAllRoles().then((res) => {
    dispayResults(res);
  });
  loadMainMenu();
}
...
async function getAllEmployeesFullData() {
  await myBLL.getAllEmployeesFullData().then((res) => {
    dispayResults(res);
  });
  loadMainMenu();
}
...
async function addDepartment() {
  await inquirer.prompt(questions.addDepartment).then(async function (answer) {
    await myBLL.addDepartment(answer.department).then((res) => {
...
async function addRole() {
  ...
  await inquirer.prompt(q).then(async function (answers) {
    let role = {
      title: answers.title,
    ...
    await myBLL.addRole(role).then((res) => {
      console.log(res);
    });
  });
...
async function updateEmployeeRole() {
  ...
  await inquirer.prompt(confirm).then(async function (answer) {
    if (answer.confirm) {
      await myBLL.updateEmployeeRole(employee, role).then((res) => {
        console.log(res);
        return;
      });
    }
  ...
  await inquirer.prompt(q).then(async function (answers) {
    ...
    let manager_id =
      answers.manager === "None"
        ? null
        : managers.find((e) => e.name === answers.manager).id;
    ...
    await myBLL.addEmployee(employee).then((res) => {
